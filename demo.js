...
window.app = app; // Expose app for other files like Notifications.js
window.db = db;
window.auth = auth;
...
window.selectSongFromSearch = (song) => {
    ...
    editorPreviewAudio.onloadedmetadata = () => {
        ...
        // Dynamically Generate Waveform Bars
...
// Base64 DataURL को Blob में बदलने के लिए
...
window.uploadStoryFromEditor = async () => {
    ...
    try {
        ...
        // यदि मीडिया वीडियो है, तो पहले वीडियो का थंबनेल कवर अपलोड करें
        ...
        await addDoc(collection(db, "stories"), {
            ...
            coverUrl: coverUrl, // यहाँ कवर इमेज का URL स्टोर होगा (वीडियो के लिए)
...
window.pauseStory = () => {
    ...
    window.stopVideoProgressLoop(); // वीडियो एनिमेशन लूप तुरंत बंद करें
...
window.resumeStory = () => {
    ...
    if(video && video.tagName === 'VIDEO') {
        video.play(); // वीडियो प्ले होते ही 'onplaying' इवेंट प्रोग्रेस बार को खुद शुरू कर देगा
    } else {
        // केवल इमेज के लिए पुराना सीएसएस ट्रांजिशन चलाएं
...
// --- स्टोरी रिप्लाई भेजने का वर्किंग फ़ंक्शन (सुरक्षित पैच अपडेट) ---
window.sendStoryReply = async (targetUid) => {
    ...
    // इनपुट फील्ड और बटन को तुरंत रीसेट करें (Instant UI feedback)
    input.value = "";
    ...
    try {
        ...
        // 1. मैसेज को फायरबेस चैट्स सब-कलेक्शन में जोड़ें (यह 100% काम करेगा क्योंकि रूम में दोनों को अनुमति होती है)
        await addDoc(collection(db, "chats", roomId, "messages"), msgData);

        // 2. खुद के यूज़र डॉक्यूमेंट को अपडेट करें (यह हमेशा अनुमत है क्योंकि यह आपका अपना UID है)
        await setDoc(doc(db, "users", currentUser.uid), { 
            lastActive: timestamp, 
            interactions: { [targetUid]: timestamp } 
        }, { merge: true });

        // 3. टारगेट यूज़र के डॉक्यूमेंट को अपडेट करें (सुरक्षित तरीके से)
        try {
            ...
            // अगर सिक्योरिटी रूल्स दूसरे यूजर के डॉक्यूमेंट में सीधे लिखने से रोकते हैं, तो भी मैसेज सेंड हो चुका है
            console.warn("Target user profile interaction bypassed by security rules:", ruleError.message);
        }

        // 4. पुश नोटिफिकेशन भेजें
        ...
        // स्टोरी को वापस चालू (Resume) करें
        window.resumeStory();
    }
};
// --- वीडियो प्रोग्रेस सिंक के लिए 60fps एनिमेशन लूप ---
...
window.startVideoProgressLoop = (videoEl, fillEl) => {
    ...
    function update() {
        if (!videoEl.paused && !videoEl.seeking && videoEl.duration) {
            // वीडियो के बिल्कुल सटीक समय के अनुसार प्रोग्रेस बार की चौड़ाई तय करें
            const pct = (videoEl.currentTime / videoEl.duration) * 100;
            fillEl.style.transition = 'none'; // लैग दूर करने के लिए CSS ट्रांजिशन बंद रखें
...
// --- स्मार्ट बैकग्राउंड प्रीलोडर (Background Prefetching Engine) ---
const prefetchedUrls = new Set(); // दोबारा डाउनलोड होने से बचाने के लिए

window.preloadNextStories = () => {
    if (!activeStoryQueue || activeStoryQueue.length === 0) return;
    
    const PRELOAD_LIMIT = 2; // वर्तमान स्टोरी से आगे कितनी स्टोरीज डाउनलोड करनी हैं
...
function prefetchMedia(url, type) {
    if (prefetchedUrls.has(url)) return; // यदि पहले से कैश हो चुका है तो दोबारा लोड न करें
    prefetchedUrls.add(url);

    if (type === 'video') {
        // वीडियो प्रीलोड: अदृश्य वीडियो टैग बनाकर बैकग्राउंड में 'auto' प्रीलोड लोड करवाएं
        ...
        videoPrefetch.load(); // ब्राउज़र को तुरंत डाउनलोडिंग शुरू करने के लिए ट्रिगर करें
    } else {
        // इमेज प्रीलोड: Image ऑब्जेक्ट बनाकर तुरंत कैश में लोड करवाएं
        const imgPrefetch = new Image();
        imgPrefetch.src = url;
    }
}
// ==========================================
// --- SHARE MODAL & SENDING LOGIC ---
// ==========================================
let shareListUids =[], filteredShareListUids =[], currentShareListIndex = 0, isFetchingShareList = false;

window.openShareModal = async (id, type, dataObj = null) => {
    // मुख्य डेटा ऑब्जेक्ट को ग्लोबल पेलोड में सुरक्षित करना
    ...
    // यदि पोस्ट टाइप है तो डेटाबेस से विवरण निकालना
    if (type === 'post') {
    ...
    if (combinedUids.size === 0 && allCachedUsers.length > 0) allCachedUsers.forEach(u => combinedUids.add(u.uid));
    combinedUids.delete(currentUser.uid); 

    shareListUids = Array.from(combinedUids); 

    // BUG FIX: जिन Followers/Following का डेटा लोकल मेमोरी (allCachedUsers) में नहीं है, उन्हें फ़ायरबेस से तुरंत प्री-कैश करना
    const missingUids = shareListUids.filter(uid => !allCachedUsers.some(u => u.uid === uid));
    if (missingUids.length > 0) {
        try {
            // फ़ायरबेस की 30-इन-क्वेरी सीमा के कारण टुकड़ों (Chunks) में डेटा डाउनलोड करना
            const chunks = [];
            ...
            for (const chunk of chunks) {
                ...
                snap.forEach(docSnap => {
                    ...
                    if (!allCachedUsers.some(u => u.uid === uData.uid)) {
                        allCachedUsers.push(uData);
                    }
                });
            }
        } catch (err) {
            console.error("Error pre-caching search users:", err);
        }
    }
    ...
    options.onscroll = () => { if (options.scrollTop + options.clientHeight >= options.scrollHeight - 50) loadMoreShareUsersList(); };
};

window.handleShareListSearch = () => {
    // सर्च बार से इनपुट लेकर उसे लोअरकेस और क्लीन करना
    const rawQuery = document.getElementById('share-list-search').value.toLowerCase().trim();
    const options = document.getElementById('share-options');
    
    if (!rawQuery) {
        filteredShareListUids = [...shareListUids]; 
    } else {
        // BUG FIX: यदि यूजरनेम सर्च करते समय '@' लगाया गया है, तो उसे हटाकर क्लीन सर्च करें
        const cleanQuery = rawQuery.startsWith('@') ? rawQuery.substring(1) : rawQuery;

        filteredShareListUids = shareListUids.filter(uid => {
            const uObj = allCachedUsers.find(u => u.uid === uid);
            if (uObj) {
                // डिस्प्ले नेम और यूजरनेम दोनों में सुरक्षित रूप से सर्च करना
...
// 1. स्टोरी में रील/पोस्ट जोड़ने का वर्किंग फ़ंक्शन
window.handleShareReelToStory = async () => {
    if (!currentUser) {
        if (typeof showCustomAlert === 'function') showCustomAlert("Auth Error", "Please login first to share.", "error");
        return;
    }
    if (!sharePayload || !sharePayload.id) {
        if (typeof showCustomAlert === 'function') showCustomAlert("Data Error", "No media selected to share.", "warning");
        return;
    }
    ...
    try {
        ...
        const storyData = {
            ...
            userPhoto: currentUserData?.avatarBase64 || currentUser?.photoURL || "https://i.pravatar.cc/150",
            mediaUrl: mediaUrl,
            mediaType: mediaType,
            coverUrl: coverUrl,
            timestamp: serverTimestamp(),
        ...
        if (typeof showToast === 'function') {
            showToast("Success", "Added to your Story!", currentUserData?.avatarBase64 || currentUser?.photoURL);
        }
        if (typeof loadStories === 'function') loadStories();
    } catch (e) {
        console.error("Error sharing to story:", e);
        if (typeof showCustomAlert === 'function') showCustomAlert("Upload Failed", "Could not share to story.", "error");
    } finally {
        if (storyBtn) {
...
// 2. बाहरी ऐप्स पर साझा करने का फ़ंक्शन
window.shareExternalPlatform = (platform) => {
    const appShareUrl = "https://www.xhp.life/?m=1";
    let shareText = "Hey! ❤️ Join me on DK Love Chats!";
    
    if (sharePayload && sharePayload.id) {
        shareText = `Hey! Check out this awesome post on DK Love Chats: ${appShareUrl}`;
    }
...
// 3. रील/पोस्ट लिंक कॉपी करने का फ़ंक्शन
window.copyCurrentItemLink = () => {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(appShareUrl).then(() => {
            if (typeof showToast === 'function') {
                showToast("Link Copied!", "App link is ready to share.", currentUserData?.avatarBase64 || currentUser?.photoURL, "success");
            }
        }).catch(() => {
            window.fallbackCopyText(appShareUrl, "App link copied!");
        });
    } else {
        window.fallbackCopyText(appShareUrl, "App link copied!");
    }
...
async function renderStoryUI() {
    ...
    // --- बैकग्राउंड प्रीलोडर सक्रिय करें (कैश डाउनलोडिंग शुरू) ---
    ...
    // --- मजबूत टच और लॉन्ग-प्रेस हैंडलिंग (पॉइंटर कैप्चर के साथ) ---
    const modalContent = document.querySelector('#story-view-modal .modal-content');
    if (modalContent) {
    ...
    // बैकग्राउंड ब्लर के लिए हमेशा 'img' का ही उपयोग करें (वीडियो लोड करने की ज़रूरत नहीं)
    ...
    // --- स्मार्ट बफरिंग और वीडियो लोडिंग सेटअप ---
    let mediaEl;
    if (story.mediaType === 'video') {
        ...
        // यदि कवर इमेज मौजूद है, तो इसे वीडियो के पोस्टर एट्रिब्यूट में लगाएँ
        if (story.coverUrl) {
        ...
        // इवेंट १: जब वीडियो बफर होने लगे
        mediaEl.onwaiting = () => {
        ...
        // इवेंट २: जब वीडियो लोड होकर चलने लगे
        mediaEl.onplaying = () => {
    ...
    if(segmentsContainer) {
        ...
        activeStoryQueue.forEach((_, idx) => {
            ...
            if (idx < currentStoryIdx) fill.style.width = '100%'; 
                ...
                // केवल इमेज के लिए ही सीएसएस ट्रांजिशन का उपयोग करें (वीडियो के लिए 60fps लूप संभालेगा)
                if (story.mediaType !== 'video') {
                    setTimeout(() => { 
...
async function loadMoreShareUsersList() {
    ...
    isFetchingShareList = true; const options = document.getElementById('share-options');
    ...
    try {
        ...
        const promises = chunk.map(async (id) => {
            let uData = allCachedUsers.find(u => u.uid === id);
            if (!uData) { const dSnap = await getDoc(doc(db, "users", id)); if (dSnap.exists()) uData = { uid: id, ...dSnap.data() }; }
            return uData;
        });
        ...
        docs.forEach(u => {
            if(u) {
                const userId = u.uid; 
                const avatar = u.avatarBase64 || u.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(u.name)}`;
                const isFollowing = currentUserData.following?.includes(userId), followsMe = currentUserData.followers?.includes(userId);
                
                let followTag = "";
                if (isFollowing && followsMe) followTag = `<span style="font-size:0.7rem; color:#aaa; border:1px solid rgba(255,255,255,0.2); padding:2px 8px; border-radius:12px; background: rgba(0,0,0,0.3);">Mutual</span>`;
                else if (isFollowing) followTag = `<span style="font-size:0.7rem; color:#00b894; border:1px solid rgba(0,184,148,0.3); padding:2px 8px; border-radius:12px; background: rgba(0,184,148,0.1);">Following</span>`;
                else if (followsMe) followTag = `<span style="font-size:0.7rem; color:#ffbe0b; border:1px solid rgba(255,190,11,0.3); padding:2px 8px; border-radius:12px; background: rgba(255,190,11,0.1);">Follows you</span>`;

                const isSelected = selectedShareUsers.has(userId);
                const selectedClass = isSelected ? 'share-user-selected' : '';
                const checkIcon = isSelected ? `<i class="fa-solid fa-circle-check check-icon" style="color: var(--success); font-size: 1.3rem;"></i>` : `<i class="fa-regular fa-circle check-icon" style="color: #555; font-size: 1.3rem;"></i>`;
        ...
        options.insertAdjacentHTML('beforeend', htmlChunk); currentShareListIndex += 20;
    } catch(e) { console.error("Error loading share users:", e); } finally { isFetchingShareList = false; }
}

window.toggleShareUser = (element, uid) => {
    const iconContainer = element.querySelector('.selection-box');
    if(selectedShareUsers.has(uid)) {
        selectedShareUsers.delete(uid); element.classList.remove('share-user-selected');
        if(iconContainer) iconContainer.innerHTML = `<i class="fa-regular fa-circle check-icon" style="color: #555; font-size: 1.3rem;"></i>`;
    } else {
        selectedShareUsers.add(uid); element.classList.add('share-user-selected');
    ...
    const btn = document.getElementById('btn-send-share');
    if(selectedShareUsers.size > 0) { btn.style.display = 'block'; btn.innerText = `Send (${selectedShareUsers.size})`; btn.style.animation = "slideUp 0.3s ease"; } 
    else btn.style.display = 'none'; 
};

window.sendBatchShare = async () => {
    if(!sharePayload || selectedShareUsers.size === 0) return;
    
    const btn = document.getElementById('btn-send-share'); btn.innerText = 'Sending...';
    const promises = [], timestamp = Date.now(); let myInteractions = {}; 
    
    selectedShareUsers.forEach(targetUid => {
        const ids = [currentUser.uid, targetUid].sort(), roomId = ids.join("_");
        ...
        const msgData = {
            text: `${textPrefix}${caption}`, mediaUrl: sharePayload.mediaUrl || sharePayload.url, mediaType: sharePayload.mediaType,
            senderId: currentUser.uid, receiverId: targetUid, seen: false, timestamp: serverTimestamp()
        };
        const ownerPhoto = sharePayload.ownerPhoto || 'https://i.pravatar.cc/150';

        if(sharePayload.type === 'post' && sharePayload.mediaType === 'video') {
            msgData.isReelShare = true; msgData.sharedReelId = sharePayload.postId || sharePayload.id; msgData.sharedReelUrl = sharePayload.mediaUrl;
            msgData.sharedReelOwnerName = sharePayload.ownerName; msgData.sharedReelOwnerPhoto = ownerPhoto; msgData.sharedReelOwnerId = sharePayload.ownerId;
        } 
        else if(sharePayload.type === 'story') {
            msgData.sharedStoryId = sharePayload.id; msgData.sharedOwnerName = sharePayload.ownerName;
            msgData.sharedOwnerPhoto = ownerPhoto; msgData.sharedOwnerId = sharePayload.ownerId;
            msgData.repliedStoryUrl = sharePayload.mediaUrl || sharePayload.url; msgData.repliedStoryType = sharePayload.mediaType || 'image';
        }
        else {
            msgData.isPostShare = true; msgData.sharedPostId = sharePayload.postId || sharePayload.id; msgData.sharedPostUrl = sharePayload.mediaUrl;
            msgData.sharedOwnerName = sharePayload.ownerName; msgData.sharedOwnerPhoto = ownerPhoto; msgData.sharedOwnerId = sharePayload.ownerId;
        }

        promises.push(addDoc(collection(db, "chats", roomId, "messages"), msgData));
        promises.push(setDoc(doc(db, "users", targetUid), { interactions: { [currentUser.uid]: timestamp } }, { merge: true }));
        myInteractions[targetUid] = timestamp;
    });
    
    promises.push(setDoc(doc(db, "users", currentUser.uid), { lastActive: timestamp, interactions: myInteractions }, { merge: true }));
    await Promise.all(promises);

    if(sharePayload.type === 'post') {
        const targetId = sharePayload.postId || sharePayload.id;
        const pRef = doc(db, "posts", targetId); const pSnap = await getDoc(pRef);
        if(pSnap.exists()) {
            const newShareCount = (pSnap.data().shareCount || 0) + selectedShareUsers.size;
            await updateDoc(pRef, { shareCount: newShareCount });
            
            const reelShareSpan = document.getElementById(`reel-share-count-${targetId}`); if (reelShareSpan) reelShareSpan.innerText = newShareCount;
            const postShareSpan = document.getElementById(`post-share-count-${targetId}`); if (postShareSpan) postShareSpan.innerText = newShareCount;
        }
    }
    
    if(typeof window.showToast === 'function') window.showToast("Sent", `Shared with ${selectedShareUsers.size} users`, currentUser.photoURL);
    window.toggleModal('share-modal', false);
};

window.copyLink = (pid) => {
    navigator.clipboard.writeText(window.location.href + '?post=' + pid);
    if(typeof window.showToast === 'function') window.showToast("Success", "Link copied to clipboard", currentUser?.photoURL);
    window.toggleModal('share-modal', false);
};
...
window.openComments = (pid) => { 
    ...
    l.innerHTML = `<div style="text-align:center; padding:20px;"><div class="splash-loader" style="width:30px;height:30px;margin:0 auto;"></div></div>`; 
    
    if(unsubscribeComments) unsubscribeComments(); 
    
    unsubscribeComments = onSnapshot(query(collection(db,"posts",pid,"comments"),orderBy("timestamp","asc")), s => { 
        l.innerHTML=""; 
        if(s.empty) l.innerHTML = `<div style="text-align:center; color:#aaa; padding:20px;">No comments yet.</div>`;
        s.forEach(d=>{ 
            const c = d.data(), cid = d.id, avatar = c.userPhoto || "https://ui-avatars.com/api/?name=" + c.userName; 
            const isLiked = c.likes && c.likes.includes(currentUser.uid);
            l.innerHTML += `
            <div class="comment-item">
                <img src="${avatar}" class="comment-avatar" onclick="if(typeof window.viewUserProfile === 'function') window.viewUserProfile('${c.userId}'); window.toggleModal('comments-modal', false);">
                <div class="comment-body">
                    <div class="comment-user" onclick="if(typeof window.viewUserProfile === 'function') window.viewUserProfile('${c.userId}'); window.toggleModal('comments-modal', false);">${c.userName}</div>
                    <div class="comment-text">${c.text}</div>
                </div>
                <div class="comment-like-container" onclick="handleLikeComment('${cid}', ${isLiked}, '${c.userId}', '${c.text}')">
                    <i class="fa-${isLiked?'solid':'regular'} fa-heart comment-like-btn ${isLiked?'liked':''}"></i><span>${c.likes ? c.likes.length : 0}</span>
                </div>
            </div>` 
        }) 
    }) 
...
window.handleLikeComment = async (commentId, isLiked, commentOwnerId, commentText) => {
    ...
    if(isLiked) await updateDoc(commentRef, { likes: arrayRemove(currentUser.uid) });
        ...
        if(typeof window.sendNotification === 'function') await window.sendNotification(commentOwnerId, 'like_comment', commentText, activeCommentPostId);
    }
};
    ...
    if(t && activeCommentPostId) { 
        ...
        try {
            await addDoc(collection(db, "posts", activeCommentPostId, "comments"), {
                text: t, userName: currentUser.displayName, userPhoto: myPhoto, userId: currentUser.uid, timestamp: serverTimestamp(), likes: []
            }); 
            ...
            if(pSnap.exists()) {
                ...
                document.querySelectorAll(`#post-comment-count-${activeCommentPostId}`).forEach(span => { span.innerText = newCount; });
...
window.fallbackCopyText = (text, successMsg) => {
    ...
    try {
        if(document.execCommand('copy') && successMsg) {
            if(typeof window.showToast === 'function') window.showToast("Copied", successMsg, currentUser?.photoURL, "success");
        }
...
window.copyAppURL = async () => {
    if (navigator.vibrate) navigator.vibrate(15);
    try {
        if (navigator.clipboard && navigator.clipboard.writeText) { await navigator.clipboard.writeText(APP_SHARE_URL); handleCopySuccess(); } 
        else throw new Error('Clipboard API unavailable');
    } catch (err) { window.fallbackCopyText(APP_SHARE_URL, "App link copied!"); window.closeAllShare(); }
};

function handleCopySuccess() {
    if(typeof window.showToast === 'function') window.showToast("Link Copied!", "App link is ready to share.", currentUserData?.avatarBase64 || currentUser?.photoURL, "success");
    window.showCustomAlert("Success", "App link copied! Now paste it on WhatsApp, Instagram or Telegram to invite friends. 🚀", "success");
    window.closeAllShare(); if (navigator.vibrate) navigator.vibrate([30, 30]);
}
...
window.shareToPlatform = async (platform) => {
    ...
    switch(platform) {
        case 'whatsapp': window.open(`https://wa.me/?text=${fullText}`, '_blank'); break;
        case 'telegram': window.open(`https://t.me/share/url?url=${APP_SHARE_URL}&text=${encodeURIComponent(APP_SHARE_TEXT)}`, '_blank'); break;
        case 'facebook': window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(APP_SHARE_URL)}`, '_blank'); break;
        case 'native':
            if (navigator.share) {
                try { await navigator.share({ title: 'DK Love Chats', text: APP_SHARE_TEXT, url: APP_SHARE_URL }); } 
                catch (e) { if (e.name !== 'AbortError') window.copyAppURL(); }
            } else window.copyAppURL();
            break;
    }
};

window.openAllShare = () => {
    const modal = document.getElementById('all-share-modal'); if (!modal) return;
    modal.style.display = 'flex';
    requestAnimationFrame(() => { modal.classList.remove('hidden'); modal.classList.add('fade-in'); if (navigator.vibrate) navigator.vibrate(15); });
};

window.closeAllShare = () => {
    const modal = document.getElementById('all-share-modal'); if (!modal) return;
    modal.classList.add('fade-out'); 
    setTimeout(() => { modal.style.display = 'none'; modal.classList.add('hidden'); modal.classList.remove('fade-out'); }, 300);
};

window.shareApp = () => { if (window.innerWidth < 768 && navigator.share) window.shareToPlatform('native'); else window.openAllShare(); };
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        // Home feed load if module is linked properly
